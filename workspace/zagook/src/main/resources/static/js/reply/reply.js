        ...
        success : function(data) {
            ...
            for(const i in data){
                ...
                if(content == ""){		// 삭제된 댓글일때
                    ...
                    if(grpl == 0){	// 모댓글일때
                        ...
                        listHtml += "		<a href='other_profile.do?other_nick="+id+"'> ";
                        listHtml += "			<img class='reply_list_profileImage' src='../images/feed/profile/"+ profile +"'/>";
                        listHtml += "		</a> ";
                        listHtml += "	</div>";
                    ...
                    if("${nick}" != ""){
                        ...
                        if("${nick}" == id){
                            ...
                            //listHtml += "			&nbsp;|&nbsp;";
                            // 삭제는 no만 넘겨주면 된다.
                            listHtml += "			<a href='javascript:' rnum='"+ rnum +"' grpl='"+ grpl + "' contentsno='"+ contentsno +"' grp='"+ grp +"' class='reply_delete'>삭제</a>";
                            listHtml += "		</div>";
                        }
                    }
                    ...
                    listHtml += "		<div class='col-1'>"
                    listHtml += "			<a href='other_profile.do?other_nick="+ id +"'> ";
                    listHtml += "				<img id='write_reply_profileImage' src='../images/feed/profile/${profile}'/>"
                    listHtml += "			</a> ";
                    listHtml += "		</div>"
                    listHtml += "		<div class='col-7'>"
                    listHtml +=  "  		<input class='w-100 input_rereply_div form-control' id='input_rereply"+ rnum +"' type='text' placeholder='댓글입력...'>"
                    listHtml += "		</div>"
                    ...
                    listHtml += "			<button type='button' class='btn btn-success mb-1 write_rereply' rnum='" + rnum + "' contentsno='" + contentsno + "'>답글&nbsp;달기</button>"
                    listHtml += "		</div>";
                    listHtml += "	</div>";
                    // ---- 답글입력란 끝
                }
            ...
            $('.reply_delete').on('click', function(){
                ...
                // 답글 삭제일때
                    ...
                    DeleteReReply($(this).attr('rnum'), $(this).attr('contentsno'), $(this).attr('grp'));
                }
...
const WriteReReply = function(contentsno,rnum) {
    ...
    if(content == ""){	// 입력된게 없을때
        ...
        $.ajax({
            ...
            success : function(pto) {
                ...
                let reply = pto.reply;
                // 페이지, 모달창에 댓글수 갱신
                $('#m_reply'+contentsno).text(reply);//
                $('#reply'+contentsno).text(reply);
...
// 모댓글 삭제일때
const DeleteReply = function(rnum, contentsno){
...
// 답글 삭제일때
const DeleteReReply = function(rnum, contentsno, grp){
